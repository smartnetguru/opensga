...
class TurmasController extends AppController
{
    ...
    function add()
    {
        ...
        $this->set(compact('anolectivos', 'estados', 'anosemestrecurr', 't0003cursos', 't0005planoestudos', 'turnos',
    ...
    public function docente_importar_pauta($turmaId)
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($type == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
                ...
                $upload_sucesso = $this->Upload->uploadFiles('uploads', array($this->request->data['Upload']['file']),
                    'pautas');
    ...
    public function docente_index()
    {
        ...
        $turmasDocente = $this->Turma->DocenteTurma->find('all', array(
            'conditions' => array(
    ...
    public function docente_print_lista_estudantes($turmaId)
    {
        ...
        $inscricaos = $this->Turma->Inscricao->find('all',
            array('conditions' => array('turma_id' => $turmaId, 'Inscricao.estado_inscricao_id' => 1)));
    ...
    public function docente_print_pauta($turmaId)
    {
        ...
        $todasTurmas = $this->Turma->find('list', array(
            'conditions' => array(
        ...
        $inscricaos2 = $this->Turma->Inscricao->find('all',
            array('conditions' => array('turma_id' => $todasTurmasIds, 'Inscricao.estado_inscricao_id' => 1)));
    ...
    public function docente_ver_turma($turmaId = null)
    {
        ...
        $turmaTipoAvaliacaos = $this->Turma->TurmaTipoAvaliacao->find('all',
            array('conditions' => array('turma_id' => $this->data['Turma']['id'])));
        $estados = array('1' => 'Activa', '2' => 'Cancelada', '3' => 'Fechada');
        $anosemestrecurr = array('1' => '1', '2' => '2', '3' => '3', '4' => '4');
        $anolectivos = $this->Turma->AnoLectivo->find('list');
        ...
        if ($this->request->is('post')) {
            ...
            if ($type == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') {
                ...
                if (isset($uploadSucesso['urls'])) {
                    ...
                    CakeResque::enqueue(
                        'default', 'TurmaShell',
                        array('processaPauta', $turmaId, $this->Upload->id, $uploadSucesso['urls'][0])
                    );
    ...
    public function faculdade_print_lista_estudantes($turma_id)
    {
        ...
        $inscricaos = $this->Turma->Inscricao->find('all',
            array('conditions' => array('turma_id' => $turma_id, 'Inscricao.estado_inscricao_id' => 1)));
        //debug($inscricaos);
        //debug();
    ...
    public function faculdade_print_pauta($turmaId)
    {
        ...
        if ($pautaFile->exists()) {
            $this->response->file($pautaFile,
                array('name' => Inflector::slug($turma['Turma']['name']), 'download' => true));
            ...
            $inscricaos2 = $this->Turma->Inscricao->find('all', array(
                'conditions' => array(
                    'turma_id' =>
                        $todasTurmasIds,
                    'Inscricao
    ...
    public function fechar_turma($id)
    {
        ...
        if (!$this->request->is('post') && !$this->request->is('put')) {
            ...
            $this->Session->setFlash(__('Não Possui Permissão para aceder esta página'), 'default', array(
                'class' => 'alert error'
            ));
        ...
        if (!$this->Turma->hasAvaliacoesAbertas($id)) {
            ...
            $this->Session->setFlash(__('Esta Turma Possui Avaliações Abertas por isso não pode ser fechada'),
                'default', array('class' => 'alert error'));
        }
    }
