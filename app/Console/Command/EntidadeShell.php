...
class EntidadeShell extends AppShell {
    ...
    public function sendSMSAniversariantes(){
        ...
        $this->Entidade->Aluno->contain(array('Entidade'=>array()));
        $alunos = $this->Entidade->Aluno->find('all',array(
            'conditions'=>array(
                'DAY(Entidade.data_nascimento)'=>date("d"),
                'MONTH(Entidade.data_nascimento)'=>date("m"),'Aluno.estado_aluno_id'=>('1,3,11,14')
            ),
                'fields'=>array('Aluno.codigo','Entidade.telemovel','Entidade.nomes','Aluno.id')
        )
        );
