...
//$this->PhpExcel->xls->getActiveSheet()->setCellValue('A4', "Ano Lectivo: ".$inscricaos[0]['Turma']['AnoLectivo']['ano'].". Semestre:".$inscricaos[0]['Turma']['semestre_curricular']);
...
//$this->PhpExcel->xls->getActiveSheet()->setCellValue('A8', "Docente:");
    ...
    foreach($candidatos as $candidato){
        ...
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('C'.$linha_actual, $candidato['Aluno']['Entidade']['name']);
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('D'.$linha_actual, $candidato['Genero']['name']);
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('E'.$linha_actual, $candidato['Aluno']['Curso']['UnidadeOrganica']['name']);
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('F'.$linha_actual, $candidato['Aluno']['Curso']['name']);
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('G'.$linha_actual, $candidato['CandidatoGraduacao']['data_defesa']);
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('H'.$linha_actual, $candidato['Aluno']['Status']);
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('I'.$linha_actual, $candidato['EstadoCandidatura']['name']);
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('J'.$linha_actual, $candidato['Aluno']['Entidade']['data_nascimento']);
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('K'.$linha_actual, $candidato['ProvinciaNascimento']['name']);
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('L'.$linha_actual, $candidato['Aluno']['Entidade']['PaisNascimento']['name']);
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('M'.$linha_actual, $candidato['FinanceiroDeposito']['valor']);
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('N'.$linha_actual, $candidato['CandidatoGraduacao']['telemovel']);
...
//$this->PhpExcel->xls->getActiveSheet()->setShowGridlines(false);
    ...
    $this->PhpExcel->output('candidatos_graduacao.xlsx');