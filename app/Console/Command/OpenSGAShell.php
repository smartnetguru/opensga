            ...
            foreach ($historicos as $historico) {
                if ($historico['HistoricoCurso']['ano_fim'] != null) {
                    if ($historico['HistoricoCurso']['nota_final'] != null) {
                        ...
                        $mudancaCurso = $this->Aluno->MudancaCurso->findByAlunoIdAndCursoAntigo($historico['HistoricoCurso']['aluno_id'],
                            $historico['HistoricoCurso']['curso_id']);
                        if ($mudancaCurso) {
                            $this->Aluno->HistoricoCurso->id = $historico['HistoricoCurso']['id'];
                    ...
                    while ($coluna <= $total_colunas) {
                        ...
                        if ($inscricao) {
                            $worksheet->setCellValueByColumnAndRow($coluna, $linha,
                                $inscricao['Inscricao']['nota_final']);
                        }
        ...
        public function exporta_candidatos_boletins()
        {
            ...
            foreach ($candidatos as $candidato) {
                $xls->getActiveSheet()->setCellValue('A' . $linha_actual,
                    $candidato['Candidatura']['numero_candidato']);
                $xls->getActiveSheet()->setCellValue('B' . $linha_actual,
...
//debug($xls->getActiveSheetIndex());
            ...
            foreach ($worksheet->getRowIterator() as $row) {
                ...
                if ($candidato) {
                    ...
                    $this->Candidatura->set('apelido',
                        ucwords(strtolower($worksheet->getCell('B' . $linha_actual)->getCalculatedValue())));
...
//   'nome_curso' => $worksheet->getCell('D' . $linha_actual)->getValue()
                ...
                if ($paisNascimento) {
                    ...
                    $this->Aluno->Entidade->PaisNascimento->set('codigo_admissao',
                        $worksheet->getCell('F' . $linha_actual)->getCalculatedValue());
                } else {
                    debug('pais');
                    die(debug($pais_nome));
                }
                ...
                if ($provinciaNascimento) {
                    ...
                    $this->Aluno->Entidade->ProvinciaNascimento->set('codigo_admissao',
                        $worksheet->getCell('H' . $linha_actual)->getCalculatedValue());
                } else {
                    debug('provincia');
...
//debug($this->Candidatura->id);
        ...
        public function importa_graduados()
        {
            AuditableConfig::$Logger = ClassRegistry::init('Auditable.Logger');
            App::import('Vendor', 'PHPExcel', ['file' => 'PHPExcel.php']);
            if (!class_exists('PHPExcel')) {
            ...
            $xls = PHPExcel_IOFactory::load(APP . 'Imports' . DS . 'graduados.xlsx');
            ...
            foreach ($worksheet->getRowIterator() as $row) {
                ...
                $numeroEstudante = $worksheet->getCell('A' . $linha_actual)->getCalculatedValue();
                $curso = $worksheet->getCell('C' . $linha_actual)->getCalculatedValue();
                $AnoConclusao = $worksheet->getCell('D' . $linha_actual)->getCalculatedValue();
                ...
                if ($numeroEstudante == '') {
                    ...
                    $nomeLike = '%' . $nomeLike . '%';
                    debug($nomeLike);
                    $entidade = $this->Aluno->Entidade->find('all',
                        ['conditions' => ['Entidade.name LIKE' => $nomeLike]]);
                    if ($entidade != 1) {
                        $linha_actual++;
                        continue;
                        debug('morreu------nome');
                        die(debug($linha_actual));
                    } else {
                        $aluno = $this->Aluno->findByEntidadeId($entidade['Entidade']['id']);
                    }
                } else {
                    $aluno = $this->Aluno->findByCodigo($numeroEstudante);
                }
                ...
                if (empty($aluno)) {
                    $xls->getActiveSheet()->setCellValue('I' . $linha_actual, 'N');
                    $linha_actual++;
                    continue;
                }
            ...
            $objWriter->save('nnnnnnnnnnnnnnn.xlsx');
            ...
            foreach ($worksheet->getRowIterator() as $row) {
                ...
                if (!empty($aluno)) {
                    if ($aluno['Aluno']['estado_aluno_id'] != 1) {
                        ...
                        $worksheet->setCellValue('N' . $linha_actual,
                            $aluno['AlunoEstado']['MotivoEstadoAluno']['name']);
...
//debug($xls->getActiveSheetIndex());
        ...
        public function nao_matriculados()
        {
            ...
            $objWriter = PHPExcel_IOFactory::createWriter($xls, 'Excel2007');
            debug(count($candidatos));
            $objWriter->save('nao_matriculados.xlsx');
        }
