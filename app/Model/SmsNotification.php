...
//Vemos se ja renovou 2014
	...
	public function getSenhaEmailEstudante($notification_id, $origin, $numero_estudante) {
		...
		$client = Zend_Gdata_ClientLogin::getHttpClient('elisio.leonardo@uem.ac.mz', 'COLOCAR SENHA AQUI', $service);
			...
			if ($is_regular[0]['regular'] == true) {
				...
				if ($codigo_activacao != 2) {
					...
					if ($conta_existe) {
						...
						$novo_email = $gdata->createUser($email_aluno, $aluno['Entidade']['nomes'], $aluno['Entidade']['apelido'], $random_password);
					}
