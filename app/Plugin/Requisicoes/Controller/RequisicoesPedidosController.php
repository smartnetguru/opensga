...
class RequisicoesPedidosController extends RequisicoesAppController {
	...
	public function index() {
		$this->paginate = array(
			...
			'contain' => array(
				'Aluno' => array(
					'Entidade', 'Curso'
				), 'Entidade', 'RequisicoesTipoPedido', 'RequisicoesEstadoPedido'
			)
		);
	...
	public function add() {
		...
		$this->set(compact('alunos', 'entidades', 'cursos', 'requisicoesTipoPedidos', 'requisicoesEstadoPedidos', 'funcionarios'));
	}
		...
		if ($this->request->is('post')) {
			if ($this->request->data['RequisicaoPedido']['tipo_pesquisa'] == 'estudante') {
				...
				if ($this->request->data['RequisicaoPedido']['codigo'] != '') {
					$estudante = $this->RequisicoesPedido->Aluno->find('first', array('conditions' => array('Aluno.codigo' => $this->request->data['RequisicaoPedido']['codigo'])));
					if (!empty($estudante)) {
						$this->Session->setFlash(__('Registe a Requisição para Este Estudante'), 'default', array('class' => 'alert alert-info'));
						$this->redirect(array('action' => 'registar_requisicao_estudante', $estudante['Aluno']['id']));
					} else {
						$this->Session->setFlash("Estudante nao encontrado", 'default', array('class' => 'alert alert-danger'));
					}
				} else {
					$this->RequisicoesPedido->Aluno->contain(array(
						'Entidade', 'Curso'
					));
					$estudantes = $this->RequisicoesPedido->Aluno->find('all', array('conditions' => array('Entidade.apelido LIKE' => '%' . $this->request->data['RequisicaoPedido']['apelido'] . '%', 'Entidade.nomes LIKE' => '%' . $this->request->data['RequisicaoPedido']['nomes'] . '%')));
					if (count($estudantes) == 1) {
						$this->Session->setFlash(__('Registe a Requisição para Este Estudante'), 'default', array('class' => 'alert info'));
						$this->redirect(array('action' => 'registar_requisicao_estudante', $estudantes[0]['Aluno']['id']));
				...
				if ($this->request->data['RequisicaoPedido']['codigo'] != '') {
					...
					if (!empty($entidade)) {
						$this->Session->setFlash(__('Registe a Requisição para Esta Entidade'), 'default', array('class' => 'alert alert-info'));
						$this->redirect(array('action' => 'registar_requisicao_entidade', $entidade['Entidade']['id']));
					} else {
						$this->Session->setFlash("Entidade nao encontrada", 'default', array('class' => 'alert alert-danger'));
					}
					...
					$entidades = $this->RequisicoesPedido->Entidade->find('all', array('conditions' => array('Entidade.apelido LIKE' => '%' . $this->request->data['RequisicaoPedido']['apelido'] . '%', 'Entidade.nomes LIKE' => '%' . $this->request->data['RequisicaoPedido']['nomes'] . '%')));
					if (count($entidades) == 1) {
						$this->Session->setFlash(__('Registe a Requisição para Este Estudante'), 'default', array('class' => 'alert info'));
						$this->redirect(array('action' => 'registar_requisicao_entidade', $entidades[0]['Entidade']['id']));
	...
	public function registar_requisicao_estudante($alunoId) {
		if ($this->request->is('post')) {
			...
			if ($this->RequisicoesPedido->save($this->request->data)) {
				...
				$this->Session->setFlash(__('Requisição Registada com Sucesso'), 'default', array('class' => 'alert success'));
				$this->redirect(array('plugin' => false, 'controller' => 'alunos', 'action' => 'perfil_estudante', $this->request->data['RequisicoesPedido']['aluno_id']));
			}
		}
...
$requisicoesEstadoPedidos = $this->RequisicoesPedido->RequisicoesEstadoPedido->find('list');
        ...
        if($this->request->is('post')) {
            ...
            if(isset($this->request->data['Entidade']['nova_entidade'])){
                ...
                //Redirecciona para action de registo de requisicao
                 $this->redirect(array('controller'=>'RequisicoesPedidos',
	...
	public function entregar_cartao_novo_ingresso($aluno_id) {
		if ($this->request->is('post')) {
			if ($this->request->data['RequisicoesPedido']['registrar_envio'] == 1) {
				...
				$requisicao = $this->RequisicoesPedido->find('first', array('conditions' => array('aluno_id' => $this->request->data['RequisicoesPedido']['aluno_id'], 'requisicoes_tipo_pedido_id' => 6)));
				$this->RequisicoesPedido->id = $requisicao['RequisicoesPedido']['id'];
				$this->RequisicoesPedido->set('requisicoes_estado_pedido_id', 6);
				$this->RequisicoesPedido->save();
				$this->Session->setFlash(__('O Envio do Cartão de Estudante foi Registado com Sucesso. Por Favor Registe o Proximo'), 'default', array('class' => 'alert success'));
				$this->redirect(array('plugin' => false, 'controller' => 'alunos', 'action' => 'pesquisa_aluno_action', 'entregar_cartao_novo_ingresso', 'requisicoes_pedidos', 'requisicoes'));
			}
		}
		...
		$this->set(compact('aluno', 'requisicoesTipoPedidos', 'requisicoesEstadoPedidos', 'is_regular', 'classe_estado'));
	}

	public function print_cartoes_enviados() {

		$this->RequisicoesPedido->contain(array(
			'RequisicoesTipoPedido', 'RequisicoesEstadoPedido', 'Aluno' => array(
				'Entidade', 'Curso' => array('UnidadeOrganica')
			)
		));
		$requisicoes = $this->RequisicoesPedido->find('first', array('conditions' => array('requisicoes_tipo_pedido_id' => 6, 'requisicoes_estado_pedido_id' => 6)));
		$this->render('index');
	}
    ...
    public function actualizar_estado_requisicao($requisicaoID){
         ...
         if($this->request->is('post')) {
             ...
             $requisicao_novo_estado = $this->request->data[
                 'RequisicoesPedido']['requisicoes_estado_pedido_id'];
             ...
             $log = array('requisicoes_pedido_id'=>$requisicaoID,
            ...
            $this->RequisicoesPedido->RequisicoesEstadoPedido->findById($requisicao_novo_estado);
             ...
             $this->redirect(array('controller'=>'requisicoes_pedidos','action'=>'index'));
            ...
            if(count($requisicao_array>0)){
                ...
                $this->set(compact(array('requisicao',
                                         'entidade',
