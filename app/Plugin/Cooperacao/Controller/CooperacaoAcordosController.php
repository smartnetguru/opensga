...
class CooperacaoAcordosController extends CooperacaoAppController
{
    ...
    public $component = ['Security', 'Paginator'];

    function beforeFilter()
    {
        $this->Security->unlockedActions = ['edit_acordo', 'novo_acordo', 'ver_acordo', 'edit_acordo_Instituicao'];
    }
...
//    }
    ...
    public function visao_geral()
    {
        ...
        $this->CooperacaoAcordo->contain(['CooperacaoTipoAcordo', 'CooperacaoArea']);
    ...
    public function nova_instituicao()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->CooperacaoAcordo->Instituicao->save($this->request->data)) {
                ...
                $this->redirect(['action' => 'lista_instituicaos']);
            } else {
    ...
    public function ver_acordo($id = null)
    {
        ...
        $options = ['conditions' => ['CooperacaoAcordo.' . $this->CooperacaoAcordo->primaryKey => $id]];
        $this->CooperacaoAcordo->contain(['CooperacaoTipoAcordo', 'CooperacaoArea']);
        $cooperacaoAcordo = $this->CooperacaoAcordo->find('first', $options);

        $optionsObjectivos = ['conditions' => ['cooperacao_acordo_id' . $this->CooperacaoAcordo->CooperacaoObjectivoAcordo->foreignKey => $id]];
        $objectivos = $this->CooperacaoAcordo->CooperacaoObjectivoAcordo->find('list', $optionsObjectivos);


        $optionsObservacao = ['conditions' => ['cooperacao_acordo_id' . $this->CooperacaoAcordo->CooperacaoAcordoObservacao->foreignKey => $id]];
        $observacao = $this->CooperacaoAcordo->CooperacaoAcordoObservacao->find('list', $optionsObservacao);

        $conditionsInst = ['conditions' => ['cooperacao_acordo_id' . $this->CooperacaoAcordoInstituicao->foreignKey => $id]];
        ...
        foreach ($instituicao as $instituicaoAc) {
            ...
            $instituicaoAcordo[] = $this->CooperacaoAcordo->Instituicao->find('all',
                ['conditions' => ['Instituicao.' . $this->CooperacaoAcordo->Instituicao->primaryKey => $instituicaoAc['CooperacaoAcordoInstituicao']['instituicao_id']]]);
        }

        $instituicoes = $this->CooperacaoAcordo->Instituicao->find('all');


        $this->set(compact('cooperacaoAreas', 'cooperacaoTipoAcordos', 'instituicoes', 'instituicaoAcordo',
            'observacao', 'objectivos', 'cooperacaoAcordo'));
    }
    ...
    public function ver_instituicao($id = null)
    {
        ...
        if (!$instituicao = $this->CooperacaoAcordo->Instituicao->getInstituicaoById($id)) {
            ...
            $options = ['conditions' => ['Instituicao.' . $this->CooperacaoAcordo->Instituicao->primaryKey => $id]];
            $this->CooperacaoAcordo->Instituicao->contain(['Continente', 'Pais', 'Provincia', 'Cidade']);
    ...
    public function edit_instituicao()
    {
        ...
        $this->CooperacaoAcordo->Instituicao->set([$name => $value, 'modified' => date('Y-m-d H:i:s')]);
    ...
    public function edit_acordo()
    {
        ...
        $this->CooperacaoAcordo->set([$name => $value, 'modified' => date('Y-m-d H:i:s')]);
...
// Funcao para operacoes sobre data
        function add_data($givendate, $day = 0, $mth = 0, $yr = 0)
        {
            ...
            $newdate = date('Y-m-d',
                mktime(0, 0, 0, date('m', $cd) + $mth, date('d', $cd) + $day, date('Y', $cd) + $yr));
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->request->data['CooperacaoAcordo']['data_validade'] != '') {
                ...
                if ($this->request->data['CooperacaoAcordo']['tempo_validade'] == 1) {
                    ...
                    $this->request->data['CooperacaoAcordo']['data_validade'] = add_data($data_, '', '',
                        $this->request->data['CooperacaoAcordo']['prazo_validade']);
            ...
            if ($this->CooperacaoAcordo->save($this->request->data)) {
                ...
                foreach ($this->request->data['CooperacaoAcordo']['instituicao_id'] as $instituicao => $id_inst) {
                    ...
                    $array_cooperacao_acordo_instituicaos = [
                        'cooperacao_acordo_id' => $this->CooperacaoAcordo->id,
                        'instituicao_id' => $id_inst,
                    ];
                ...
                $this->Session->setFlash(__('Problemas ao gravar dados. Por favor, Tente novamente'), 'default',
                    ['class' => 'alert error']);
            }
        }
