    ...
    <table cellpadding="0" cellspacing="0">
        ...
        <?php foreach ($roles as $role): ?>
            <tr>
                ...
                <td><?php echo h($role['Role']['modified_by']); ?>&nbsp;</td>
                <td>
                    <?php echo $this->Html->link($role['EstadoObjecto']['name'],
                            ['controller' => 'estado_objectos', 'action' => 'view', $role['EstadoObjecto']['id']]); ?>
                </td>
                <td class="actions">
                    <?php echo $this->Html->link(__('View'), ['action' => 'view', $role['Role']['id']]); ?>
                    <?php echo $this->Html->link(__('Edit'), ['action' => 'edit', $role['Role']['id']]); ?>
                    <?php echo $this->Form->postLink(__('Delete'), ['action' => 'delete', $role['Role']['id']],
                            ['confirm' => __('Are you sure you want to delete # %s?', $role['Role']['id'])]); ?>
                </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
