...
foreach ($bolseiros as $curso => $bolsas) {
    ...
    if ($pagina != 1) {
        $excel->mergeCells('G' . $linha_actual . ':J' . $linha_actual);
        $excel->getStyle('G' . $linha_actual . ':J' . $linha_actual)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $excel->duplicateStyle($this->PhpExcel->xls->getActiveSheet()->getStyle('G' . $linha_assinatura), 'G' . $linha_actual.':J'.$linha_actual);
...
//$this->PhpExcel->addImage(WWW_ROOT . 'img' . DS . 'logo_login_' . Configure::read('OpenSGA.instituicao.sigla') . '.png', 'A29', 10, 108, 115); //Logotipo
    ...
    if ($pagina != 1) {
        ...
        $excel->duplicateStyle($this->PhpExcel->xls->getActiveSheet()->getStyle('A' . $linha_dra), 'A' . $linha_actual);
    ...
    $linha_actual++;
    $linha_actual++;
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('A' . $linha_actual, 'ATRIBUIÇÃO DE BOLSAS DE ESTUDO, ISENÇÃO E REDUÇÃO DE PROPINAS');
    if ($pagina != 1) {
        ...
        $excel->duplicateStyle($this->PhpExcel->xls->getActiveSheet()->getStyle('A' . $linha_titulo), 'A' . $linha_actual);
    ...
    if ($pagina != 1) {
        $excel->mergeCells('A' . $linha_actual . ':I' . $linha_actual);
        $excel->getStyle('A' . $linha_actual . ':I' . $linha_actual)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $excel->duplicateStyle($this->PhpExcel->xls->getActiveSheet()->getStyle('A' . $linha_sub), 'A' . $linha_actual);
        $excel->getStyle('A' . $linha_actual . ':I' . $linha_actual)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
    }
    $linha_actual++;
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('A' . $linha_actual, 'Ano Lectivo de 2015');
    if ($pagina != 1) {
        $excel->mergeCells('A' . $linha_actual . ':I' . $linha_actual);
        $excel->getStyle('A' . $linha_actual . ':I' . $linha_actual)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $excel->duplicateStyle($this->PhpExcel->xls->getActiveSheet()->getStyle('A' . $linha_ano), 'A' . $linha_actual);
        $excel->getStyle('A' . $linha_actual . ':I' . $linha_actual)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
    }
  /*  $this->PhpExcel->xls->getActiveSheet()->setCellValue('E' . $linha_actual, '2015 ');
    ...
    if ($pagina != 1) {
        $excel->mergeCells('A' . $linha_actual . ':I' . $linha_actual);
        $excel->getStyle('A' . $linha_actual . ':I' . $linha_actual)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $excel->duplicateStyle($this->PhpExcel->xls->getActiveSheet()->getStyle('A' . $linha_curso), 'A' . $linha_actual . ':I' . $linha_actual);
        $excel->getStyle('A' . $linha_actual . ':I' . $linha_actual)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
    }
    ...
    if ($pagina != 1) {
        ...
        $excel->duplicateStyle($this->PhpExcel->xls->getActiveSheet()->getStyle('F' . $linha_table), 'F' . $linha_actual);
    ...
    if ($pagina != 1) {
        ...
        $excel->duplicateStyle($this->PhpExcel->xls->getActiveSheet()->getStyle('H' . $linha_table), 'H' . $linha_actual);
    ...
    foreach ($bolsas as $bolsa) {
        ...
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('C' . $linha_actual, $bolsa['BolsaTemporaria']['nomes']);
        if ($pagina != 1) {
            $excel->mergeCells('C' . $linha_actual . ':E' . $linha_actual);
            $excel->getStyle('C' . $linha_actual . ':E' . $linha_actual)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
        ...
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('H' . $linha_actual, $bolsa['BolsaTemporaria']['doador']);
        if ($pagina != 1) {

            $excel->getStyle('H' . $linha_actual)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
            $excel->getStyle('H' . $linha_actual)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
        }
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('I' . $linha_actual, $bolsa['BolsaTemporaria']['alinea']);
        if ($pagina != 1) {
    ...
    $linha_actual++;
    $linha_actual++;
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('A' . $linha_actual, 'Data: '.date('27/02/2015 H:i:s'));
    $excel->mergeCells('A' . $linha_actual . ':C' . $linha_actual);
    $excel->getStyle('A' . $linha_actual . ':C' . $linha_actual)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
    $excel->getStyle('A' . $linha_actual . ':C' . $linha_actual)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
    //$this->PhpExcel->xls->getActiveSheet()->setCellValue('D' . $linha_actual, $total);
