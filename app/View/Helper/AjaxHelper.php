...
class AjaxHelper extends AppHelper
{
     ...
     * @var array
     */
    var $helpers = ['Html', 'Form', 'Javascript'];
     ...
     */
    function link($title, $href = null, $options = [], $confirm = null, $escapeTitle = true)
    {
        if (!isset($href)) {
            $href = $title;
        }
     ...
     * @access private
     */
    function __getHtmlOptions($options, $extra = [])
    {
        ...
        foreach ($options as $key => $value) {
            switch ($key) {
                ...
                case 'form':
                    ...
                    break;
                default:
                    if (!in_array($key, $this->callbacks) && !in_array($key,
                            ['before', 'after', 'confirm', 'condition', 'interactive', 'update'])
                    ) {
                        $jsOptions[$key] = $value;
                    }
                    break;
     ...
     * @return array
     */
    function _optionsToString($options, $stringOpts = [])
    {
     ...
     * @see AjaxHelper::link()
     */
    function form($params = null, $type = 'post', $options = [])
    {
        $model = false;
        if (is_array($params)) {
     ...
     * @see AjaxHelper::form()
     */
    function submit($title = 'Submit', $options = [])
    {
     ...
     */
    function observeField($field, $options = [])
    {
        if (!isset($options['with'])) {
     ...
     * @return string ajax script
     */
    function observeForm($form, $options = [])
    {
        if (!isset($options['with'])) {
        ...
        return $this->Javascript->event("'#{$form} input, #{$form} select, #{$form} textarea'", "change",
            $callback);
    }
    ...
    function _buildUICallbacks($options, $ui_calbacks = [])
    {
        $callbacks = [];
     ...
     */
    function dropRemote($id, $options = [], $ajaxOptions = [])
    {
        $var = '';
        if (isset($options['var'])) {
            $var = 'var ' . $options['var'] . ' = ';
     ...
     */
    function selectable($id, $options = [])
    {
        if (isset($options['var'])) {
            $var = 'var ' . $options['var'] . ' = ';
            unset($options['var']);
     ...
     */
    function editor($id, $url, $options = [])
    {
        $url = $this->url($url);
