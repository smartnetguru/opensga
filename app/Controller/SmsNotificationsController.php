<?php
	...
	class SmsNotificationsController extends AppController
	{
		...
		public function get_message()
		{
			...
			switch ($comando_recebido) {
				case "rm2014":
					...
					$this->SmsNotification->processaSMSRenovacao($this->SmsNotification->id, $origin,
						$message_explode[1]);

					break;
				case "crm2014":

					$this->SmsNotification->processaSMSConfirmacaoRenovacao($this->SmsNotification->id, $origin,
						$message_explode[1]);
				...
				case "email":
					...
					break;
				case "senha":
					$this->SmsNotification->getSenhaEmailEstudante($this->SmsNotification->id, $origin,
						$message_explode[2]);
					break;
				case "ojm":
		...
		public function send_sms_estrangeiros()
		{
			...
			$candidatos = $this->Aluno->find('all', ['conditions' => ['ano_ingresso' => 2015]]);
			...
			$this->SmsEnviada->sendSMS(826489374,
				"A UEM, o convida a cerimónia de lançamento da iniciativa Alumni no dia //22/08/2014 no Centro Cultural e niversitário, pelas 14.00 horas.");
			$this->SmsEnviada->sendSMS(822454375,
				"A UEM, o convida a cerimónia de lançamento da iniciativa Alumni no dia //22/08/2014 no Centro Cultural e niversitário, pelas 14.00 horas.");
			$this->SmsEnviada->sendSMS(845179795,
				"A UEM, o convida a cerimónia de lançamento da iniciativa Alumni no dia //22/08/2014 no Centro Cultural e niversitário, pelas 14.00 horas.");
			...
			$this->SmsEnviada->sendSMS(826489374, "Todas SMS foram enviadas.");
			$this->SmsEnviada->sendSMS(844213948,
				"A UEM, o convida a cerimónia de lançamento da iniciativa Alumni no dia 22/08/2014, no Centro Cultural e Universitário,pelas 14.00 horas.");
		}
