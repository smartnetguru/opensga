...
//die(debug($aluno));
    $this->PhpExcel->loadWorksheet(APP . 'Reports' . DS . Alunos . DS . 'exportar_alunos_autenticidades.xlsx');
...
//$this->PhpExcel->xls->getActiveSheet()->setCellValue('A8', "Docente:");
    ...
    foreach ($alunos as $candidato) {
        ...
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('D' . $linha_actual,
            $candidato['Entidade']['Genero']['name']);
        $this->PhpExcel->xls->getActiveSheet()->setCellValue('E' . $linha_actual,
            $candidato['Curso']['UnidadeOrganica']['name']);
    ...
    $this->PhpExcel->output('alunos_nivel_anterior.xlsx');
