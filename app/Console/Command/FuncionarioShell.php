    ...
    public function importa_funcionarios()
    {
        ...
        foreach ($worksheet->getRowIterator() as $row) {
            ...
            if (!empty($entidadeExiste)) {
                    ...
                    if ($isDocente) {
                        ...
                        if (!empty($ocenteExiste)) {
                            ...
                            if (empty($docenteUnidadeExiste)) {
                                $arrayDocenteUnidadeOrganica = ['docente_id'          => $docenteExiste['Docente']['id'],
                            ...
                            $arrayDocenteUnidadeOrganica = ['docente_id'          => $this->Funcionario->Entidade->Docente->id,
                                                            'unidade_organica_id' => $unidadeOrganicaId,
                                                            'estado_objecto_id'   => 1
                            ];
                            $this->Funcionario->Entidade->Docente->DocenteUnidadeOrganica->create();
                ...
                if(!$this->Funcionario->User->save(['User'=>['username'=>$username,'password'=>$password,'group_id'=>$groupId]])){
                    $dataSource->rollback();
                    debug($username);
                    debug($nomes);
                    debug($apelido);
