<div class="faculdadeTipoPagamentos index">
	<h2><?php echo __('Faculdade Tipo Pagamentos');?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id');?></th>
			<th><?php echo $this->Paginator->sort('faculdade_id');?></th>
			<th><?php echo $this->Paginator->sort('tipopagamento_id');?></th>
			<th><?php echo $this->Paginator->sort('valor');?></th>
			<th class="actions"><?php echo __('Actions');?></th>
	</tr>
	<?php
	foreach ($faculdadeTipoPagamentos as $faculdadeTipoPagamento): ?>
	<tr>
		<td><?php echo h($faculdadeTipoPagamento['FaculdadeTipoPagamento']['id']); ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($faculdadeTipoPagamento['Faculdade']['name'], array('controller' => 'faculdades', 'action' => 'view', $faculdadeTipoPagamento['Faculdade']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($faculdadeTipoPagamento['Tipopagamento']['name'], array('controller' => 'tipopagamentos', 'action' => 'view', $faculdadeTipoPagamento['Tipopagamento']['id'])); ?>
		</td>
		<td><?php echo h($faculdadeTipoPagamento['FaculdadeTipoPagamento']['valor']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $faculdadeTipoPagamento['FaculdadeTipoPagamento']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $faculdadeTipoPagamento['FaculdadeTipoPagamento']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $faculdadeTipoPagamento['FaculdadeTipoPagamento']['id']), null, __('Are you sure you want to delete # %s?', $faculdadeTipoPagamento['FaculdadeTipoPagamento']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>

	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Faculdade Tipo Pagamento'), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Faculdades'), array('controller' => 'faculdades', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Faculdade'), array('controller' => 'faculdades', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Tipopagamentos'), array('controller' => 'tipopagamentos', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Tipopagamento'), array('controller' => 'tipopagamentos', 'action' => 'add')); ?> </li>
	</ul>
</div>
