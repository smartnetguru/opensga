...
//die(debug($aluno));
    ...
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('N15',
        $aluno['Entidade']['EstadoCivil']['name']); //Estado Civil
    ...
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('B21',
        $aluno['Entidade']['PaisNascimento']['name']); //Pais de Nascimento
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('H21',
        $aluno['Entidade']['ProvinciaNascimento']['name']); //Provincia de Nascimento
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('N21',
        $aluno['Entidade']['CidadeNascimento']['name']); //Cidade de Nascimento
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('E23',
        $aluno['Entidade']['data_nascimento']); //Data de Nascimento
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('I23', $aluno['Entidade']['Genero']['name']); //Sexo
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('M23',
        $aluno['Entidade']['PaisNascimento']['name']); //Nacionalidade
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('G25',
        $aluno['Entidade']['DocumentoIdentificacao']['name'] . '/' . $aluno['Entidade']['documento_identificacao_num']); //Numero de Documento de Identificacao
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('O25',
        $aluno['Entidade']['documento_identificacao_validade']); //Data de Validade
    ...
    foreach ($inscricaos as $inscricao) {
        if (!in_array($linha_actual, [71, 72, 106, 107])) {
            ...
            $this->PhpExcel->xls->getActiveSheet()->setCellValue('K' . $linha_actual,
                $inscricao['Inscricao']['nota_frequencia']); //Apelido
            $this->PhpExcel->xls->getActiveSheet()->setCellValue('L' . $linha_actual, ''); //Apelido
            $this->PhpExcel->xls->getActiveSheet()->setCellValue('M' . $linha_actual,
                $inscricao['Inscricao']['nota_exame_normal']); //Apelido
            $this->PhpExcel->xls->getActiveSheet()->setCellValue('N' . $linha_actual, ''); //Apelido
            $this->PhpExcel->xls->getActiveSheet()->setCellValue('O' . $linha_actual,
                $inscricao['Inscricao']['nota_exame_recorrencia']); //Apelido
            $this->PhpExcel->xls->getActiveSheet()->setCellValue('P' . $linha_actual,
                $inscricao['Inscricao']['nota_final']); //Apelido
    ...
    if ($has_foto_entidade) {
        $this->PhpExcel->addImage(APP . 'Assets' . DS . 'Fotos' . DS . 'Estudantes' . DS . $aluno['Aluno']['codigo'] . '.JPG',
            'N1', 5, 180, 175); //Fotografia
    } else {
        $this->PhpExcel->addImage(WWW_ROOT . 'img' . DS . 'default_profile_picture.jpg', 'N1', 5, 180,
            175); //Fotografia
    }
    $this->PhpExcel->addImage(WWW_ROOT . 'img' . DS . 'logo_login_' . Configure::read('OpenSGA.instituicao.sigla') . '.png',
        'A1', 10, 220, 145); //Logotipo
    $this->PhpExcel->xls->getActiveSheet()->setCellValue('A7',
