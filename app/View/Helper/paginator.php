...
class PaginatorHelper extends AppHelper
{
    ...
    function __construct($config = [])
    {
        ...
        if (!is_callable([$classname, 'link'])) {
            trigger_error(sprintf(__('%s does not implement a link() method, it is incompatible with PaginatorHelper',
     ...
     * @access public
     */
    function prev($title = '<< Previous', $options = [], $disabledTitle = null, $disabledOptions = [])
    {
     ...
     * @access protected
     */
    function __pagingLink($which, $title = null, $options = [], $disabledTitle = null, $disabledOptions = [])
    {
        $check = 'has' . $which;
        $_defaults = [
            'url' => [],
        ...
        $url = array_merge(['page' => $paging['page'] + ($which == 'Prev' ? $step * -1 : $step)], $url);

        if ($this->{$check}($model)) {
            return $this->Html->tag($tag,
                $this->link($title, $url, array_merge($options, compact('escape', 'class'))));
        } else {
     ...
     * @access public
     */
    function url($options = [], $asArray = false, $model = null)
    {
     ...
     * @access public
     */
    function sortKey($model = null, $options = [])
    {
        if (empty($options)) {
     ...
     * @access public
     */
    function next($title = 'Next >>', $options = [], $disabledTitle = null, $disabledOptions = [])
    {
     ...
     * @access public
     */
    function sort($title, $key = null, $options = [])
    {
     ...
     * @access public
     */
    function sortDir($model = null, $options = [])
    {
        $dir = null;
            ...
            if ($first && $start > 1) {
                ...
                if ($offset < $start - 1) {
                    ...
                    $out .= $this->first($offset,
                        ['tag' => $tag, 'after' => $separator, 'separator' => $separator]);
                }
            }
            ...
            if ($last && $end < $params['pageCount']) {
                ...
                if ($offset <= $last && $params['pageCount'] - $end > $offset) {
                    ...
                    $out .= $this->last($offset,
                        ['tag' => $tag, 'before' => $separator, 'separator' => $separator]);
                }
            }
     ...
     * @access public
     */
    function first($first = '<< first', $options = [])
    {
        $options = array_merge(
            [
                'tag' => 'span',
     ...
     * @access public
     */
    function last($last = 'last >>', $options = [])
    {
        $options = array_merge(
            [
                'tag' => 'span',
