    ...
    class FinanceiroTransacaosController extends AppController
    {
        ...
        public function novo_deposito($aluno_id)
        {
            ...
            if ($this->request->is('post') || $this->request->is('put')) {
                ...
                if ($deposito) {
                    ...
                    $this->redirect(['controller' => 'alunos', 'action' => 'perfil_estudante', $aluno_id]);
                } else {
        ...
        public function novo_pagamento($aluno_id)
        {
            if ($this->request->is('post') || $this->request->is('put')) {
                ...
                if ($this->FinanceiroTransacao->processarDeposito($this->request->data)) {
                    ...
                    $this->request->data['FinanceiroPagamento']['entidade_id'] = $this->FinanceiroTransacao->FinanceiroPagamento->Aluno->field('entidade_id',
                        ['Aluno.id' => $aluno_id]);
                    ...
                    if ($pagamento_id) {
                        ...
                        if ($this->FinanceiroTransacao->pagarFactura($pagamento_id, $aluno_id)) {
                            $this->Session->setFlash(__('O Pagamento foi efectuado com Sucesso'), 'default',
                                ['class' => 'alert success']);
                        }
                    } else {
                        $this->Session->setFlash(__('Houve um problema ao registar o Pagamento Seleccionado. Mas o depósito do valor foi efectuado',
                            'default', ['class' => 'alert error']));
                    }
                } else {
                    $this->Session->setFlash(__('Os dados do deposito efectuados não estão correctos. Por favor, verifique',
                        'default', ['class' => 'alert error']));
                }
            }

            $saldo_actual = $this->FinanceiroTransacao->getSaldoActual($aluno_id);
            $total_divida = $this->FinanceiroTransacao->FinanceiroPagamento->find('all', [
                'conditions' => ['aluno_id' => $aluno_id, 'financeiro_estado_pagamento_id' => 1],
                'fields'     => 'SUM(FinanceiroPagamento.valor) as valor',
            ]);

            $pagamentos_pendentes = $this->FinanceiroTransacao->FinanceiroPagamento->find('all',
                ['conditions' => ['aluno_id' => $aluno_id, 'financeiro_estado_pagamento_id' => 1]]);

            $pagamentos_efectuados = $this->FinanceiroTransacao->FinanceiroPagamento->find('all', [
                'conditions' => [
                    'aluno_id'                       => $aluno_id,
            ...
            $financeiro_tipo_pagamentos = $this->FinanceiroTransacao->FinanceiroPagamento->FinanceiroTipoPagamento->find('list',
                ['conditions' => ['codigo >' => 21]]);

            $this->FinanceiroTransacao->Entidade->Aluno->contain([
                'Entidade',
            ]);
            $aluno = $this->FinanceiroTransacao->Entidade->Aluno->findById($aluno_id);


            $this->set(compact('saldo_actual', 'pagamentos_pendentes', 'total_divida', 'financeiro_tipo_pagamentos',
                'aluno_id', 'pagamentos_efectuados', 'aluno'));
        }
        ...
        public function pagar_factura($pagamento_id, $aluno_id)
        {
            if ($this->request->is('post')) {
                ...
                $this->redirect(['controller' => 'financeiro_transacaos', 'action' => 'novo_pagamento', $aluno_id]);
            } else {
