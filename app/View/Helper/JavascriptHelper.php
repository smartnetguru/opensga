...
class JavascriptHelper extends AppHelper
{
     ...
     */
    function codeBlock($script = null, $options = [])
    {
     ...
     */
    function event($object, $event, $observer = null, $options = [])
    {
        $isObject = (
     ...
     */
    function includeScript($script = "", $options = [])
    {
        if ($script == "") {
            $files = scandir(JS);
            $javascript = '';
     ...
     */
    function value2($val, $quoteStrings = true)
    {
        switch (true) {
        ...
        if ($this->useNative) {
            ...
            foreach ($data as $key => $val) {
                if (is_array($val) || is_object($val)) {
                    $val = $this->object($val, array_merge($options, ['block' => false]));
                } else {
                    $quoteStrings = (
     ...
     * @return string
     */
    function writeEvents($inline = true, $options = [])
    {
        $out = '';
        $rules = [];
